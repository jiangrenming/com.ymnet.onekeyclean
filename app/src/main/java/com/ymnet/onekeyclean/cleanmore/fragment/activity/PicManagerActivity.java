...
public class PicManagerActivity extends ImmersiveActivity implements View.OnClickListener {
    ...
    private void initView(Context context) {
        ...
        tvTopTitle.setText(R.string.picture_clean);
        ...
        noDocument.setText(R.string.image_dir_empty);
        mInfos = new ArrayList<FileInfo>();
        deleteMap = new HashMap<Integer, FileInfo>();
        adapter = new FileItemAdapter(context, mInfos, deleteMap);
        recyclerView.setLayoutManager(new GridLayoutManager(context, FileManagerInfo.PIC_COUNT));
        recyclerView.addItemDecoration(new SpaceItemDecoration(FileManagerInfo.SPACE));
    ...
    public class SpaceItemDecoration extends RecyclerView.ItemDecoration {
        ...
        public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
            ...
            outRect.left = space;
            outRect.top = space;
            if (parent.getChildLayoutPosition(view) % FileManagerInfo.PIC_COUNT == 0) {
                outRect.left = space;
            }

            if (parent.getChildLayoutPosition(view) % FileManagerInfo.PIC_COUNT == FileManagerInfo.PIC_COUNT - 1) {
                outRect.right = space;
            }
        }
    }
    ...
    private void initData(final Context context) {
        ...
        AsyncTaskwdh<Void, Void, Void> task = new AsyncTaskwdh<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                ArrayList<FileInfo> infos = fc.getAllPicList();//文件夹对应的图片
                ...
                while (iterator.hasNext() && mInfos != null) {
                    ...
                    if (next != null) {
                        boolean b = FileBrowserUtil.deleteOtherFile(context, next.getValue().fileId,
                                next.getValue().filePath, FileCategoryHelper.FileCategory.Picture);
                        if (b) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUESTCODE_IMAGE) {
            mInfos = (ArrayList<FileInfo>) data.getSerializableExtra("infos");
            if (adapter != null) {
                adapter.setDate(mInfos, deleteMap);
            }
        }
