...
/**
     ...
     * <li>else see {@link #installNormal(Context, DownloadInfo)}</li>
     * </ul>
     *
     * @param context
     * @param filePath
...
/**
    ...
    public static boolean installNormal(Context context, String filePath) {
        return installNormal(context, filePath, true);
    }


    public static boolean installNormal(Context context, String filePath, boolean enableAccessibility) {
        ...
        DownloadLog.downloadEvent(DownloadLog.START_INSTALL, filePath);

        startInstall(context, filePath, enableAccessibility);
        return true;
    }


    public static boolean installNormal(Context context, DownloadInfo downloadInfo) {
        ...
        DownloadLog.downloadEvent(DownloadLog.START_INSTALL, downloadInfo.mId);

        startInstall(context, downloadInfo.mFileName, true);

        return true;
    }

    public static final String URI_KEY = "uri_key";
    public static final String SHOW_KEY = "show_install_tip_key";


    public static void startInstall(Context context, String filePath) {
        startInstall(context, filePath, true);
    }

    public static void startInstall(Context context, String filePath, boolean enableAccessibility) {
        if (null == context || null == filePath) {
            return;
        }
        if (enableAccessibility) {
            if (AccessibilityUtil.needShowAccessibilityGuide()) {
                AccessibilityUtil.showAccessibilityDialog(filePath);
                return;
            }

            AppInfo info = Utils.getApkInfo(context, filePath);
            if (info != null) {
                AccessibilityUtil.addAppName(info.getAppName());
            }
        }
        Intent i = new Intent();
...
//        }
    ...
    */
/**
     * 判断MiUi Settings app中是否存在DeviceAdminFragment.class
     *
     * @return 如果存在返回true，否则返回false
     *//*

    private static boolean isClzExist() {
        try {
            Intent intent = new Intent("android.intent.action.MAIN");
            ComponentName cm = new ComponentName("com.android.settings", "com.android.settings.Settings");
            intent.setComponent(cm);

            PackageManager pm = C.get().getPackageManager();
            ...
            if (!activities.isEmpty()) {
                ...
                String outDir = C.get().getApplicationInfo().dataDir;

                String libraryDir = null;
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD) {
                    libraryDir = activityInfo.applicationInfo.nativeLibraryDir;
                }

                DexClassLoader loader = new DexClassLoader(sourceDir, outDir, libraryDir, C.get().getClass().getClassLoader());
                loader.loadClass("com.android.settings.security.DeviceAdminFragment");
                return true;
            }
