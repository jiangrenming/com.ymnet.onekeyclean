...
public class ProgressWheel extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //计算View的大小并调用setMeasuredDimension()设置View的大小
        if (widthWithoutPadding > heigthWithoutPadding) {
            size = heigthWithoutPadding;
        } else {
            size = widthWithoutPadding;
        }
        // 如果重写了onMeasure()方法，必须调用setMeasuredDimension()方法
        // 这是设置View大小的唯一途径
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < splitText.length; i++) {
            ...
            canvas.drawText(splitText[i], this.getWidth() / 2 - horizontalTextOffset,
                    this.getHeight() / 2 + verticalTextOffset * (count - 1), textPaint);
            count += 3;
        }
        if (isSpinning) {
    ...
    public void incrementProgress() {
        ...
        if (progress > 360)
            progress = 0;
        int round = Math.round((progress / 360) * 100);
        String source = String.valueOf(round);
        SpannableString ss = new SpannableString(source);
        ss.setSpan(new AbsoluteSizeSpan(24, true), 0, source.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        setText(ss + "%\n已用");
